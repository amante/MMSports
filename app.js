/* MMSports v1.3.0 */
const STATE_KEYS = {
  players: 'mmsports_players',
  cartillas: 'mmsports_cartillas'
};
...
function coerceHeaderVariants(rows) {
  // Normalize common variants/accents
  return rows.map(r => {
    const o = {};
    for (const k in r) {
...
function parseCartillasRows(rows) {
  ...
  rows.forEach(row => {
    ...
    if (actual!==null) {
      if (condicion.replace(/\s+/g,'') === '>=') {
        cumple = actual >= objetivo;
        restante = Math.max(0, objetivo - actual);
      } else if (condicion.replace(/\s+/g,'') === '<=') {
        cumple = actual <= objetivo;
        restante = Math.max(0, actual - objetivo);
      }
    }

    byId.get(cartilla).lineas.push({ jugador, estadistica, condicion: condicion.replace(/\s+/g,''), objetivo, actual, restante, cumple });
  });
...
// Robust CSV to rows (handles quotes and commas)
...
function nextSlide() {
  if (!cartillas.length) return;
  currentSlide = (currentSlide + 1) % cartillas.length;
  renderCarousel();
}
function prevSlide() {
  if (!cartillas.length) return;
  currentSlide = (currentSlide - 1 + cartillas.length) % cartillas.length;
  renderCarousel();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#btnExport').addEventListener('click', () => {
    const payload = { players, cartillas, version: '1.3.0', exportedAt: new Date().toISOString() };
    downloadJSON('mmsports_estado.json', payload);
  });
